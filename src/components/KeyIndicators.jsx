...
export default function KeyIndicators({ data, historicalData }) {
    ...
    const mandatoryExpenses = data.expense.szemelyi + data.expense.koh + data.expense.dologi + data.expense.jarulek;
            ...
            <div className="grid-charts" style={{ marginTop: '1.5rem' }}>
                ...
                <div className="glass-panel" style={{ padding: '1.5rem', height: '400px', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    <div className="responsive-header">
                        <div>
                            <h4 style={{ color: 'var(--text-muted)' }}>Helyi Adók arányai és alakulása (millió Ft) ({data.year}) {data.yoy && formatYoY(data.yoy.taxTotal)}</h4>
                    ...
                    <div style={{ flex: 1, minHeight: 0 }}>
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={taxTrendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                ...
                                <Bar dataKey="Idegenforgalmi adó" stackId="a" fill="purple" radius={[0, 0, 0, 0]} />
